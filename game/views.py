...
def index(request):
    ...
    for y in range(1, grid_size - 1):
        for x in range(1, grid_size - 1):
            if grid[y][x] == 1:
                ...
                if (grid[y][x-1] == 0 and grid[y][x+1] == 0) or (grid[y-1][x] == 0 and grid[y+1][x] == 0):
                    walls.append((x, y))
    ...
    path_cells = [(x, y) for y in range(grid_size) for x in range(grid_size) if grid[y][x] == 0]
    if (0, 0) in path_cells: path_cells.remove((0, 0))
    if (grid_size-1, grid_size-1) in path_cells: path_cells.remove((grid_size-1, grid_size-1))
